<script>
  ...
  // 先存 track，防止 LIFF 登入後 URL 參數消失
  const urlTrack = new URLSearchParams(window.location.search).get('track');
  if (urlTrack) sessionStorage.setItem('liff_track', urlTrack);
  const track = sessionStorage.getItem('liff_track') || 'lycra_free';
  ...
  btnSubmit.addEventListener('click', async function() {
    ...
    try {
      ...
      if (data.success) {
        sessionStorage.removeItem('liff_track');
